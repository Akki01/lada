...
from lada.gui.fullscreen_mouse_activity_controller import (
    FullscreenMouseActivityController,
)
...
from lada.gui.frame_restorer_provider import (
    FrameRestorerProvider,
    FrameRestorerOptions,
    FRAME_RESTORER_PROVIDER,
)
...
class VideoPreviewView(Gtk.Widget):
    ...
    def __init__(self, **kwargs):
        ...
        self._buffer_queue_min_thresh_time_auto_min = 2.0
        self._buffer_queue_min_thresh_time_auto_max = 8.0
        self._buffer_queue_min_thresh_time_auto = (
        ...
        self.widget_timeline.connect(
            "seek_requested",
            lambda widget, seek_position: self.seek_video(seek_position),
        )
        self.widget_timeline.connect(
            "cursor_position_changed",
            lambda widget, cursor_position: self.show_cursor_position(cursor_position),
        )
    ...
    def toggle_button_preview_video_callback(self, button_clicked):
        if self._video_preview_init_done:
            self.frame_restorer_options = self._frame_restorer_options.with_passthrough(
                not self._frame_restorer_options.passthrough
            )
    ...
    def button_play_pause_callback(self, button_clicked):
        ...
        if self.pipeline_manager.state == PipelineState.PLAYING:
            ...
            logger.warning(
                f"unhandled pipeline state in button_play_pause_callback: {self.pipeline_manager.state}"
            )
    ...
    def frame_restorer_options(self, value: FrameRestorerOptions):
        if self._frame_restorer_options == value:
            return
        if (
            self._video_preview_init_done
            and self._buffer_queue_min_thresh_time == 0
            and self._frame_restorer_options.max_clip_length != value.max_clip_length
        ):
            self.buffer_queue_min_thresh_time_auto = float(
                value.max_clip_length / value.video_metadata.video_fps_exact
            )
    ...
    def buffer_queue_min_thresh_time_auto(self, value):
        value = min(
            ...
            max(self._buffer_queue_min_thresh_time_auto_min, value),
        )
    ...
    def setup_config_signal_handlers(self):
        def on_preview_mode(*args):
            if self._frame_restorer_options:
                self.frame_restorer_options = (
                    self._frame_restorer_options.with_mosaic_detection(
        ...
        def on_device(object, spec):
            if self._frame_restorer_options:
                self.frame_restorer_options = self._frame_restorer_options.with_device(
                    self._config.device
                )
        ...
        def on_mosaic_restoration_model(object, spec):
            if self._frame_restorer_options:
                self.frame_restorer_options = (
                    self._frame_restorer_options.with_mosaic_restoration_model_name(
                        self._config.mosaic_restoration_model
                    )
                )

        self._config.connect(
            "notify::mosaic-restoration-model", on_mosaic_restoration_model
        )

        def on_mosaic_detection_model(object, spec):
            if self._frame_restorer_options:
                self.frame_restorer_options = (
                    self._frame_restorer_options.with_mosaic_detection_model_name(
                        self._config.mosaic_detection_model
                    )
                )

        self._config.connect(
            "notify::mosaic-detection-model", on_mosaic_detection_model
        )

        self._config.connect(
            "notify::preview-buffer-duration",
            lambda object, spec: self.set_property(
                "buffer-queue-min-thresh-time", object.get_property(spec.name)
            ),
        )
        ...
        def on_max_clip_duration(object, spec):
            if self._frame_restorer_options:
                self.frame_restorer_options = (
                    self._frame_restorer_options.with_max_clip_length(
    ...
    def update_gst_buffers(self):
        buffer_queue_min_thresh_time, buffer_queue_max_thresh_time = (
            self.get_gst_buffer_bounds()
        )
        self.pipeline_manager.update_gst_buffers(
            buffer_queue_min_thresh_time, buffer_queue_max_thresh_time
        )
    ...
    def _open_file(self, file: Gio.File):
        ...
        self.frame_restorer_options = FrameRestorerOptions(
            self.config.mosaic_restoration_model,
            self.config.mosaic_detection_model,
            video_utils.get_video_meta_data(self.opened_file.get_path()),
            self.config.device,
            self.config.max_clip_duration,
            self.config.preview_mode == "mosaic-detection",
            False,
        )
        file_path = file.get_path()

        self.video_metadata = video_utils.get_video_meta_data(file_path)
        self._frame_restorer_options = self._frame_restorer_options.with_video_metadata(
            self.video_metadata
        )
        self.has_audio = (
        ...
        self.file_duration_ns = int(
            (self.video_metadata.frames_count * self.frame_duration_ns)
        )
        self._buffer_queue_min_thresh_time_auto_min = (
            float(
        ...
        self.buffer_queue_min_thresh_time_auto = (
            self._buffer_queue_min_thresh_time_auto_min
        )
        ...
        if self.pipeline_manager:
            # Warm‑Start: Pipeline bleibt bestehen
            self.pipeline_manager.set_source_file(self.video_metadata)
        else:
            buffer_queue_min_thresh_time, buffer_queue_max_thresh_time = (
                self.get_gst_buffer_bounds()
            )
            ...
            self.pipeline_manager.connect(
                "waiting-for-data",
                lambda obj, waiting_for_data: self.on_waiting_for_data(
                    waiting_for_data
                ),
            )
            self.pipeline_manager.connect(
                "notify::state",
                lambda object, spec: self.on_pipeline_state(
                    object.get_property(spec.name)
                ),
            )
    ...
    def on_eos(self, *args):
        self.eos = True
        self.button_image_play_pause.set_property(
    ...
    def on_waiting_for_data(self, waiting_for_data):
        ...
        if waiting_for_data:
            ...
            if (
                self._buffer_queue_min_thresh_time == 0
                and self._video_preview_init_done
            ):
                self.buffer_queue_min_thresh_time_auto *= 1.2
                self.update_gst_buffers()
                GLib.timeout_add(1500, lambda: self._restore_buffer_threshold())

        else:
            if not self.should_be_paused:
                self.pipeline_manager.play()

    def _restore_buffer_threshold(self):
        self.buffer_queue_min_thresh_time_auto = (
            self._buffer_queue_min_thresh_time_auto_min
        )
        self.update_gst_buffers()
    ...
    def get_gst_buffer_bounds(self):
        buffer_queue_min_thresh_time = (
            self._buffer_queue_min_thresh_time
            if self._buffer_queue_min_thresh_time > 0
            else self._buffer_queue_min_thresh_time_auto
        )
        buffer_queue_max_thresh_time = buffer_queue_min_thresh_time * 1.3
    ...
    def get_time_label_text(self, time_ns):
        if not time_ns or time_ns == -1:
            ...
            time = (
                ...
                if hours == 0
                else f"{hours}:{minutes:02d}:{seconds:02d}"
            )
            return time
    ...
    def on_fullscreened(self, fullscreened: bool):
        if fullscreened:
            self.fullscreen_mouse_activity_controller = (
                FullscreenMouseActivityController(self)
            )
            self.banner_no_gpu.set_revealed(False)
            self.button_toggle_fullscreen.set_property(
        ...
        self.fullscreen_mouse_activity_controller.connect(
            "notify::fullscreen-activity",
            lambda object, spec: self.on_fullscreen_activity(
                object.get_property(spec.name)
            ),
        )
    ...
    def _setup_shortcuts(self):
        def on_shortcut_preview_toggle(*args):
            if self.stack_video_preview.get_visible_child_name() == "video-player":
                self.toggle_button_preview_video_callback(
                    self.toggle_button_preview_video
                )

        self._shortcuts_manager.register_group("files", "Files")
        self._shortcuts_manager.add(
            "files",
            "export-file",
            "e",
            lambda *args: self.show_export_dialog(),
            "Export recovered video",
        )
        ...
        self._shortcuts_manager.add(
            ...
            "toggle-mute-unmute",
            "m",
            lambda *args: self.button_mute_unmute_callback(self.button_mute_unmute),
            "Mute/Unmute",
        )
        self._shortcuts_manager.add(
            "preview",
            "toggle-play-pause",
            "<Alt>space",
            lambda *args: self.button_play_pause_callback(self.button_play_pause),
            "Play/Pause",
        )
        self._shortcuts_manager.add(
            "preview",
        ...
        self._shortcuts_manager.add(
            ...
            "toggle-fullscreen",
            "<Ctrl>f",
            lambda *args: self.emit("toggle-fullscreen-requested"),
    ...
    def show_export_dialog(self):
        ...
        file_dialog.set_initial_name(
            f"{os.path.splitext(self.opened_file.get_basename())[0]}.restored.mp4"
        )
        file_dialog.save(
            callback=lambda dialog, result: self.emit(
                "video-export-requested", self.opened_file, dialog.save_finish(result)
            )
        )
